...
import 'package:adamulti_mobile_clone_new/model/redeem_komisi_response.dart';
...
class KomisiService {
  ...
  Future<RedeemKomisiResponse> redeemKomisi(String uuid) async {
    final decodeTokenResult = await locator.get<JwtService>().decodeToken();

    final response = await _dio.post("$baseUrlCore/komisi", data: {
      "uuid": uuid
    }, 
    options: Options(
      headers: {
    ...
    return RedeemKomisiResponse.fromJson(response.data);
  }
}