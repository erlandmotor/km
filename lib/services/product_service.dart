...
class ProductService {
  ...
  Future<PricelistGroupResponse> getPriceListGroup(String uuid) async {
    final decodeTokenResult = await locator.get<JwtService>().decodeToken();

    final response = await _dio.post("$baseUrlCore/pricelistgroup", 
    data: {
      "uuid": uuid,
    },
    options: Options(
    ...
    return PricelistGroupResponse.fromJson(response.data);
  }

  Future<PricelistSearchResponse> getPriceLisSearch(String uuid, String cari) async {
    final decodeTokenResult = await locator.get<JwtService>().decodeToken();

    final response = await _dio.post("$baseUrlCore/pricelist", 
    data: {
      "uuid": uuid,
      "cari": cari
    },
    ...
    return PricelistSearchResponse.fromJson(response.data);
  }
}