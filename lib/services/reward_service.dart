...
class RewardService {
  ...
  Future<RewardExchangeResponse> getRewardList(String uuid) async {
    final decodeTokenResult = await locator.get<JwtService>().decodeToken();
    final response = await _dio.get("$baseUrlCore/hadiah/reward?uuid=$uuid", options: Options(
      headers: {
        'Content-Type': 'application/json',
        'x-auth-irs': decodeTokenResult
      },
    ...
    return RewardExchangeResponse.fromJson(response.data);
  }
